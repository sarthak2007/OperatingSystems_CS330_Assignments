#include<ulib.h>

int main(u64 arg1, u64 arg2, u64 arg3, u64 arg4, u64 arg5)
{

  int pages = 4096;

  char * mm1 = mmap(NULL, pages*6, PROT_READ|PROT_WRITE, 0);
  if((long)mm1 < 0)
  {
    // Testcase failed.
     printf("Test case failed \n");
    return 1;
  }
  // vm_area count should be 1.
  pmap(0);
  pmap(1);

  // int pages = 4096;

  char * mm2 = mmap(mm1+10*pages, pages*6, PROT_READ|PROT_WRITE, 0);
  if((long)mm2 < 0)
  {
    // Testcase failed.
     printf("Test case failed \n");
    return 1;
  }
  // vm_area count should be 1.
  pmap(0);
  pmap(1);
  unsigned long readonly_address = (unsigned long)mm1 + pages* 3;

  // Should change access rights of the third page, existing vm_Area should be splitted up. A new vm area with access rights with PROT_READ will be created.
  int result  = mprotect((void *)readonly_address, pages, PROT_READ);

  if(result <0)
  {
    // Testcase failed
     printf("Test case failed \n");
    return 0;
  }
  
  // vm_area count should be 3.
  pmap(0);
  pmap(1);
  unsigned long invalid_Address =  (unsigned long)mm1 + pages* 50;

  // mprotect should fail.
  int result1  = mprotect((void *)invalid_Address, pages,PROT_READ);

  if(!(result1 <0))
  {
    // Testcase failed
     printf("Test case failed \n");
    return 0;
  }

  //vm_area count should be 3
  pmap(0);
  pmap(1); 

  readonly_address = (unsigned long)mm1 + pages* 4;

  // Should change access rights of the third page, existing vm_Area should be splitted up. A new vm area with access rights with PROT_READ will be created.
  result  = mprotect((void *)readonly_address, pages, PROT_READ);

  if(result <0)
  {
    // Testcase failed
     printf("Test case failed \n");
    return 0;
  }
  
  // vm_area count should be 3.
  pmap(0);
  pmap(1);

  readonly_address = (unsigned long)mm1 ;

  // Should change access rights of the third page, existing vm_Area should be splitted up. A new vm area with access rights with PROT_READ will be created.
  result  = mprotect((void *)readonly_address, pages*2, PROT_WRITE);

  if(result <0)
  {
    // Testcase failed
     printf("Test case failed \n");
    return 0;
  }
  
  // vm_area count should be 3.
  pmap(0);
  pmap(1);

  readonly_address = (unsigned long)mm1 +3*pages ;

  // Should change access rights of the third page, existing vm_Area should be splitted up. A new vm area with access rights with PROT_READ will be created.
  result  = mprotect((void *)readonly_address, pages*2, PROT_WRITE|PROT_READ);

  if(result <0)
  {
    // Testcase failed
     printf("Test case failed \n");
    return 0;
  }
  
  // vm_area count should be 3.
  pmap(0);
  pmap(1);
 return 0;
}
