...
//gives page table entry corresponding to given virtual address
...
//unmaps the physical page if pte exists corresponding to given virtual address
...
//change flags in pte if pte exists
...
//allocates page frame if MAP_POPULATE flag was passed in mmap
...
int vm_area_mprotect(struct exec_context *current, u64 addr, int length, int prot)
{
	...
	while(curr!=NULL){
		if(addr < curr->vm_start){
			if(addr+length > curr->vm_start && addr+length <= curr->vm_end){
				span++;break;
			}
	...
	//if only one vm_area is intersected by address range
	if(span == 1){
		curr = current->vm_area;
		prev = NULL;
		while(curr!=NULL){
			if(addr == curr->vm_start){
				if(addr+length == curr->vm_end){
					...
					if(curr->access_flags == prot || (prev!=NULL && prot == prev->access_flags))	break;
					if(cnt >= 128)	return -1;					
				}
			}
			else if(addr > curr->vm_start && addr < curr->vm_end){
				if(addr+length == curr->vm_end){
					if(curr->access_flags == prot || (curr->vm_next!=NULL && prot == curr->vm_next->access_flags))	break;
					if(cnt >= 128)	return -1;	
				}
				else{
	...
	//if two vm_area's are intersected by address range
	else if(span == 2){
		curr = current->vm_area;
		prev = NULL;
		while(curr!=NULL){
			if(addr == curr->vm_start){
				...
				if(addr+length == curr->vm_next->vm_end){
					break;	
				}
				else{
					if(curr->access_flags == prot || curr->vm_next->access_flags == prot)	break;
					if(cnt >= 128)	return -1;
				}
			}
			prev = curr;
	...
	//first allocate without any merging
	curr = current->vm_area;
	prev = NULL;
	flag=0;
	while(curr!=NULL){
	...
	//hint address is provided
		...
		if(ret==-1 && flag==1){
			if(flags&MAP_FIXED)
				...
				//hint address was already mapped
				...
				if(ret==-1){
					if(curr!=NULL && curr->vm_end + length <= MMAP_AREA_END){
						if(curr->access_flags == prot){
							curr->vm_end += length;
							ret = curr->vm_end - length;
						}
						...
						if(curr->vm_start > length + start){
							...
							//start from beginning to search for blank space
							while(curr->vm_next!=NULL && curr->vm_end < addr){
								if(curr->vm_end + length < curr->vm_next->vm_start){
									if(curr->access_flags == prot){
										curr->vm_end += length;
										ret = curr->vm_end - length;
...
int vm_area_unmap(struct exec_context *current, u64 addr, int length)
{
	...
	while(curr!=NULL){
		// flag = 0 means address range has not started yet
		if(flag==0){
